...
from MBVMusic.utils.database import add_served_chat, get_assistant
            ...
            [
                [
                    InlineKeyboardButton(
                        "ƨσʋяcɛ", url=f"https://github.com/FantasticSukhi/MBVMUSIC"
                    )
                ]
            ]
        ),
...
async def add_allbot(client, message):
    ...
    if len(command_parts) != 2:
        await message.reply(
            "**⚠️ ɪɴᴠᴀʟɪᴅ ᴄᴏᴍᴍᴀɴᴅ ғᴏʀᴍᴀᴛ. ᴘʟᴇᴀsᴇ ᴜsᴇ ʟɪᴋᴇ » `/gadd @MBV_MUSIC_GBOT`**"
        )
        return
